...
pemdir = Path(__file__).parent

now = datetime.now(timezone.utc)
VALIDITY = timedelta(minutes=2922)
...
(pemdir/'ca-pubkey.pem').write_bytes(ca_pubkey.public_bytes(
    Encoding.PEM, PublicFormat.SubjectPublicKeyInfo))
(pemdir/'ca-cert.pem').write_bytes(ca_cert.public_bytes(Encoding.PEM))
...
(pemdir/'server-privkey.pem').write_bytes(server_privkey.private_bytes(
    Encoding.PEM, PrivateFormat.PKCS8, NoEncryption()))
(pemdir/'server-pubkey.pem').write_bytes(server_pubkey.public_bytes(
    Encoding.PEM, PublicFormat.SubjectPublicKeyInfo))
(pemdir/'server-cert.pem').write_bytes(server_cert.public_bytes(Encoding.PEM))
...
(pemdir/'client-privkey.pem').write_bytes(client_privkey.private_bytes(
    Encoding.PEM, PrivateFormat.PKCS8, NoEncryption()))
(pemdir/'client-pubkey.pem').write_bytes(client_pubkey.public_bytes(
    Encoding.PEM, PublicFormat.SubjectPublicKeyInfo))
(pemdir/'client-cert.pem').write_bytes(client_cert.public_bytes(Encoding.PEM))
